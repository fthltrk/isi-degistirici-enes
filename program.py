...
class EGRGelis_mis_Laboratuvar(ctk.CTk):
    def __init__(self):
        ...
        self.title("EGR Cooler Ar-Ge ve Termal Analiz Laboratuvarı v4.7")
    ...
    def setup_sekme_sihirbaz(self):
        ...
        ctk.CTkLabel(
            box,
            text="Bu panelde gireceğiniz boru geometrisi verileri, ana simülasyondaki Isı Transfer Alanını (A) dinamik belirler.",
        ...
        # DÜZELTİLMİŞ: width parametresi CTkButton tanımında (pack içinde değil)
        btn_aktar = ctk.CTkButton(
            box, text="Hesapla ve Alana Aktar 💾",
            font=("Arial", 13, "bold"), width=250,
            command=self.sihirbazdan_aktar, fg_color="#e67e22", hover_color="#d35400"
        )
    ...
    def sihirbazdan_aktar(self):
        try:
            ...
            if d <= 0 or L <= 0 or N <= 0:
                raise ValueError("Tüm boyut değerleri sıfırdan büyük olmalıdır.")
            ...
            messagebox.showerror("Geometri Hatası", f"Girilen değer hatası: {str(e)}")
        except Exception:
    ...
    def sema_ciz(self, T_ei, T_si, T_eo, T_so, akis_tipi):
        ...
        # DÜZELTİLMİŞ: negatif y koordinatı yerine canvas içinde görünür konuma taşındı
        self.canvas_sema.create_text(65, 15, text=f"Egzoz Giriş\n{T_ei:.1f}°C", fill="#ff7675", font=("Arial", 10, "bold"))
        ...
        self.canvas_sema.create_text(535, 15, text=f"Egzoz Çıkış\n{T_eo:.1f}°C", fill="#f39c12", font=("Arial", 10, "bold"))
 
        # Akış Yönüne Göre Sıvı Okları
        if "Zıt" in akis_tipi:
            self.canvas_sema.create_rectangle(435, 35, 465, 72, fill="#2c3e50", outline="#7f8c8d", width=1)
            self.canvas_sema.create_line(450, 35, 450, 72, fill="#3498db", width=5, arrow="first")
            self.canvas_sema.create_text(450, 20, text=f"Sıvı Giriş: {T_si:.1f}°C", fill="#74b9ff", font=("Arial", 9, "bold"))
 
            self.canvas_sema.create_rectangle(125, 148, 155, 185, fill="#2c3e50", outline="#7f8c8d", width=1)
            self.canvas_sema.create_line(140, 148, 140, 185, fill="#1abc9c", width=5, arrow="last")
            self.canvas_sema.create_text(140, 200, text=f"Sıvı Çıkış: {T_so:.1f}°C", fill="#55efc4", font=("Arial", 9, "bold"))
 
            self.canvas_sema.create_text(295, 218, text="⬅ AKIŞ YÖNÜ: ZIT AKIŞ (COUNTER)", fill="#74b9ff", font=("Arial", 9, "bold"))
        else:
            self.canvas_sema.create_rectangle(125, 35, 155, 72, fill="#2c3e50", outline="#7f8c8d", width=1)
            self.canvas_sema.create_line(140, 35, 140, 72, fill="#3498db", width=5, arrow="last")
            self.canvas_sema.create_text(140, 20, text=f"Sıvı Giriş: {T_si:.1f}°C", fill="#74b9ff", font=("Arial", 9, "bold"))
 
            self.canvas_sema.create_rectangle(435, 148, 465, 185, fill="#2c3e50", outline="#7f8c8d", width=1)
            self.canvas_sema.create_line(450, 148, 450, 185, fill="#1abc9c", width=5, arrow="last")
            self.canvas_sema.create_text(450, 200, text=f"Sıvı Çıkış: {T_so:.1f}°C", fill="#55efc4", font=("Arial", 9, "bold"))
 
            self.canvas_sema.create_text(295, 218, text="➡ AKIŞ YÖNÜ: PARALEL AKIŞ (CO-CURRENT)", fill="#e74c3c", font=("Arial", 9, "bold"))
